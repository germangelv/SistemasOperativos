	...
	if (argc > 1) {
		...
		if ( pid_amigo != 0 ) {
			...
			while (i<CANTMSJS) {
				...
				fseek(fp, 0, SEEK_SET);
				size_t by;
				by = fwrite(mensaje, sizeof(char), TAMBUF, fp);
				...
				reactivar = 0;
				//fgets(mensaje, TAMBUF, fp); // Leo archivo creado por el otro proceso
				by = fread(mensaje, sizeof(char), TAMBUF, fp);
				fflush(fp); //Fuerzo volcado de archivo
				//printf("%d-", by);
				//fflush(stdout);
			...
			fflush(stdout);
			printf("\033[H\033[J");
			printf("\033[1;33m\n\t *** Estadisticas Caso 1: Archivo sincronizado con signals *** \n");
