...
BUSCAREXECUTABLESDIR(){
	...
	for file in `ls -l "$1"` ; do
		...
		IFS=$'\n'
		intCountArray=${#arrFileFields[@]}
		if [[ intCountArray -gt 5 ]] ;then 
			#Arreglo espacios en el titulo
			intPosNombreIni=8
			intPosNombreActual=$intPosNombreIni
			strFileName=""
			while [ $intPosNombreActual -lt $intCountArray ] ; do
				if [[ $intPosNombreIni != $intPosNombreActual ]] ; then
					strFileName="$strFileName "
				fi
				strFileName="$strFileName${arrFileFields[$intPosNombreActual]}"
				intPosNombreActual=$(($intPosNombreActual+1))
			done
			#echo `file "$1/$strFileName"`
			if [[ `file "$1/$strFileName"` = *"executable"* ]] ; then
				intCantTotal=$(($intCantTotal + 1))
				if [[ $blReporteYear -eq 0 ]] ; then
					#echo $file
					if [[ -z ${hshDirReporte["${1}"]} ]] ; then 
						arrDirReporteOrder+=("$1")
					fi
					hshDirReporte["${1}"]=$((${hshDirReporte["${1}"]} + 1))
				else
					intPosNombreAnio=7
					if [[ $blDebug -eq 1 ]] ; then
						echo " ->$file"
						echo " ->${arrFileFields[$intPosNombreAnio]} - $strFileName"
					fi
					strAnio=$strAnioActual
					if [[ ${arrFileFields[intPosNombreAnio]} != *":"* ]] ; then
						strAnio=${arrFileFields[$intPosNombreAnio]}
					fi
