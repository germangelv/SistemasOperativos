...
# Silva, Heber Ezequiel             DNI 36.404.597         
#
#*****************************************************
...
#valido si el usuario le puso la barra de directorio al final del parametro, si no es así la pongo.
if [[ "${parametro: -1}" != '/'  ]]; then
	parametro="$parametro/";
fi;
...
{ 	
   while [ true ]; do
	...
	miVector=("${miVector[@]}" $(ls "$parametro"*.* | rev | cut -d'.' -f1 | rev | uniq ));
